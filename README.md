...
docker compose up -d
cp .env.example .env
# fill TREASURY_SECRET, TRENDER_API_KEY, XAI_API_KEY, SOLANA_RPC_URL, Spaces, SESSION_SECRET, SITE_PASSWORD
npm install
...
The HTML site (`/`, `/mint`, `/c/:ticker`) is gated by `SITE_PASSWORD`. Sign in at `/login`. GrokBot `POST /v1/launch` still uses `TRENDER_API_KEY`. Use `/mint` to name a coin, attach a post and socials, upload a picture or generate one from a description, then mint.
